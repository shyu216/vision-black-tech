...
for l in range(nlevels):
    if l == 0 or l == nlevels - 1:
        print(f"Apply alpha: {alpha} to level {l} ({pyr[l].shape[1]}x{pyr[l].shape[0]})")
        continue
    ...
    if currAlpha > alpha:
        ...
        print(f"Apply alpha: {currAlpha} to level {l} ({pyr[l].shape[1]}x{pyr[l].shape[0]})")
    
    lambda_ /= 2

fig, ax = plt.subplots()
depth_queues = [deque(maxlen=100) for _ in range(2)]
labels = ['Origin', 'Lap Pyr Amplified']
...
def update(frame):
    ...
    ax.set_ylim(0, max(max(depth_queues[0]), max(depth_queues[1])) * 2)
