...
fig, ax = plt.subplots()
intensity_queues = [deque(maxlen=100) for _ in range(2)]
...
def update(frame):
    ...
    cv2.putText(color_image, f"ROI Average Intensity: {roi_intensity:.4f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    ax.set_ylim(0, max(max(intensity_queues[0]), max(intensity_queues[1])) * 2)
